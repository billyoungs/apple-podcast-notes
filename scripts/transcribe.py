#!/usr/bin/env python3
"""
transcribe.py — 把音频转成文字。支持多种后端：

阿里云百炼（DashScope，同一个 DASHSCOPE_API_KEY）：
  ...
  paraformer(模型 paraformer-v2，便宜 + 说话人分离 + 每月免费 10h)
      python transcribe.py --from-meta ./_work --out ./_work --backend funasr --diarize [--speaker-count 2]

SenseAudio（OpenAI 兼容，需先下载音频）：
      export ASR_API_KEY="<senseaudio-token>"
      python transcribe.py ./_work/audio.m4a --out ./_work --backend senseaudio \\
          --model senseaudio-asr-lite-1.5-260319 --diarize --speaker-count 2

通用 api（其它 OpenAI 兼容 ASR）：
      export ASR_API_BASE=...; export ASR_API_KEY=...
      python transcribe.py ./_work/audio.m4a --out ./_work --backend api --model whisper-large-v3

音频来源：阿里云后端用公网 URL（推荐 --from-meta 自动读 meta.json 的 audio_url，无需下载本地）。
          SenseAudio/通用 API 后端需先 --download 下载音频为本地文件。
...
# ---------------- 后端：SenseAudio（OpenAI 兼容，支持说话人分离+热词） ----------------
SENSEAUDIO_DEFAULT_MODEL = "senseaudio-asr-lite-1.5-260319"
SENSEAUDIO_BASE = "https://api.senseaudio.cn/v1"


def run_senseaudio(audio, out_dir, model, language, diarize, speaker_count, hotwords):
    key = os.environ.get("ASR_API_KEY", "")
    if not key:
        sys.exit("[缺配置] 请设置 ASR_API_KEY（SenseAudio API Token）")
    if audio.lower().startswith("http"):
        sys.exit("[用法错误] SenseAudio 后端需本地文件，请先用 --download 下载音频。")
    url = f"{SENSEAUDIO_BASE}/audio/transcriptions"
    print(f"[转录] SenseAudio {model}")
    if diarize:
        print("    说话人分离=开")
    if hotwords:
        print(f"    热词={hotwords}")

    boundary = "----senseaudio" + str(int(time.time()))
    with open(audio, "rb") as f:
        audio_bytes = f.read()
    parts = []
    ...
    if diarize:
        ...
        if speaker_count:
            field("max_speakers", str(speaker_count))
    if hotwords:
        field("hotwords", hotwords)
    ...
    try:
        ...
        sys.exit(f"[错误] SenseAudio API HTTP {e.code}: {body}")
    ...
    if segs:
        ...
        for s in segs:
            text = s.get("text", "") or ""
            spk = s.get("speaker")
            if diarize and spk is not None and spk != last_spk:
                text = f"【{spk}】{text}"
                last_spk = spk
            collected.append((s.get("start", 0), s.get("end", 0), text))
    if not collected:
        collected = [(0, 0, data.get("text", ""))]
    write_outputs(out_dir, collected)
...
def main():
    ...
    ap.add_argument("--backend", choices=["qwen", "funasr", "paraformer", "senseaudio", "api"],
                    default="qwen")
    ap.add_argument("--model", default=None,
                    help="qwen默认 qwen3-asr-flash-filetrans；funasr默认 fun-asr；paraformer默认 paraformer-v2；senseaudio默认 senseaudio-asr-lite-1.5-260319；api默认 whisper-large-v3")
    ap.add_argument("--language", default="zh")
    ap.add_argument("--region", choices=["cn", "intl"], default="cn",
                    help="云端地域：cn=北京，intl=新加坡（API Key 不同）")
    # funasr / paraformer / senseaudio 专用
    ap.add_argument("--diarize", action="store_true", help="funasr/paraformer/senseaudio: 开启说话人分离")
    ...
    def _run(backend):
        if backend == "qwen":
            ...
            run_funasr(audio, args.out, args.model or "paraformer-v2", args.language,
                       args.region, args.diarize, args.speaker_count, args.vocabulary_id)
        elif backend == "senseaudio":
            run_senseaudio(audio, args.out, args.model or SENSEAUDIO_DEFAULT_MODEL,
                           args.language, args.diarize, args.speaker_count, args.hotwords)
        else:
